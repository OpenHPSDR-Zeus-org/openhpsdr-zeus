...
- **🚫 PureSignal integrity (P1 *and* P2) — PARAMOUNT.** No change may break or regress PureSignal on **either** protocol. PS feedback, adaptive predistortion, and the TX→IQ→feedback path are load-bearing. Any change that touches DSP, TX, drive/PA, the TX IQ ring, threading/locks, audio ingest, or the WDSP P/Invoke seam MUST keep PS **byte-identical at defaults** and pass the PS golden tests — `Zeus.Server.Tests` (`~Ps` / `~PureSignal` / `~TwoTone`) **and** `Zeus.Protocol2.Tests` (`~PsWireFormat`) — **before it ships**. If a change *can* affect PS and you cannot PS-bench-verify it (G2 for P2, HL2 for P1), say so explicitly in the PR so the maintainer bench-tests first. This overrides convenience, scope, and schedule — when in doubt, protect PS.
...
When uncertain, implement the minimal fix and note in the PR description that design decisions need maintainer review. **Brian (EI6LF) and Doug (KB2UKA) are the authorities on visual design, UX, and defaults** — either can make or approve these calls; nothing is deferred to Brian alone. Doug routinely makes UX/visual/default decisions directly (e.g. menu structure, control behavior), so "red-light" means "needs Brian's or Doug's sign-off," not "parked on Brian."
