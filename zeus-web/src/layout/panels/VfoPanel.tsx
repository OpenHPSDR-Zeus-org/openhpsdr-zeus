...
export function VfoPanel() {
  ...
  // Per-RX listen/mute. RX1 is the audio clock-master; muting it drops it from
  // the mix but it keeps clocking. Optimistic so the lane reacts immediately.
  ...
  const audibleOf = (index: number) =>
    receivers.find((r) => r.index === index)?.audible ?? true;
