...
export function createVfoNudgeController(receiver: SpectrumReceiver = 'A'): VfoNudgeController {
  ...
  const writeVfo = (hz: number) => optimisticSetReceiverVfo(receiver, hz);
  const postVfo = (hz: number, signal?: AbortSignal) =>
    postReceiverVfo(receiver, hz, signal);
  const tunesOffCenter = () => receiverIsSecondary || useConnectionStore.getState().ctunEnabled;
...
export function usePanTuneGesture(
  ...
  useEffect(() => {
    ...
    const tuneIsSecondary = tuneReceiver !== 'A';
    const panIsSecondary = receiver !== 'A';
    ...
    const writeVfo = (hz: number) => optimisticSetReceiverVfo(tuneReceiver, hz);
    const postVfo = (hz: number, signal?: AbortSignal) =>
    ...
    // centre IS the VFO, so a pan writes/posts that receiver's VFO. RX1 pans the
    // hardware NCO (radioLoHz / setRadioLo).
    ...
    // Read back the applied pan centre from a server state response: secondary
    // receivers report it on their own VFO field (vfoBHz for 'B', receivers[]
    // entry for RX3+); RX1 on radioLoHz.
    const readAppliedPanHz = (state: RadioStateDto): number => {
      if (!panIsSecondary) return state.radioLoHz;
    ...
    const flushPanPending = () => {
      ...
      postPanCenter(hz, ctrl.signal)
        .then((state) => {
          ...
          reconcileAppliedPan(readAppliedPanHz(state));
        })
        .catch(() => {});
    };
    ...
    const onPointerUp = (e: PointerEvent) => {
      ...
      if (d.moved) {
        ...
        if (target.snappedToSignal) {
          ...
          if (target.fromLive && !tuneIsSecondary && receiver === 'A') {
            armSnapLock({
